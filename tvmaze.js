		...
		let $item = $(
         ...
         <div class="card" data-show-id="${show.id}">
           ...
           <img class="card-img-top" src="${show.image}">
             ...
             <button id="idButt" class="btn btn-primary"> Episodes </button>
           </div>
         </div>
       </div>
      `
...
$('#shows-list').on('click', async function(e) {
	e.preventDefault();
	let targetElement = e.target.type;
	let selector = 'submit';
	while (targetElement != null) {
		if (targetElement.match(selector)) {
			let targetShow = e.target.closest('div.card');

			let showID = $(targetShow).data('show-id');

			let temp = await getEpisodes(showID);
			return populateEpisodes(temp);
		} else return;
	}
});
